package.path = package.path .. ";./Libs/LibDeflate/?.lua"
    ...
    local function serializeValue(v)
        if type(v) == "table" then
            ...
            for k, val in pairs(v) do
                ...
                table.insert(entries, key .. " = " .. serializeValue(val))
            end
...
local function GenerateDatabase()
    local outputPath = "C:/Games/World of Warcraft/_classic_era_/Interface/AddOns/PocketMoney/EncryptedAchievements.lua"
    local output = io.open(outputPath, "w")
    
    if not output then
        print("Error: Cannot open file at " .. outputPath)
        return false
    end
    
    output:write("-- Generated achievements database\n")
    
    local serialized = SerializeTable(achievements)
    local compressed = LibDeflate:CompressDeflate(serialized)
    local encoded = LibDeflate:EncodeForWoWAddonChannel(compressed)
    local checksum = CalculateChecksum(encoded)
 
    output:write(string.format("PocketMoneyEncryptedAchievements = %q\n", encoded))
    output:write("PocketMoneyChecksum = " .. checksum .. "\n")
    output:close()
    
    print("Successfully generated achievements file")
    return true
end
